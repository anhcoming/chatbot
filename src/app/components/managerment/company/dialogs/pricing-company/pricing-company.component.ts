...
@Component({
  selector: 'app-pricing-company',
  templateUrl: './pricing-company.component.html',
  styleUrls: ['./pricing-company.component.scss']
})
export class PricingCompanyComponent {
  ...
  getServicePricingById(id: number) {
    this.servicePricingService.getServicePricingById(id).subscribe((res: ResApi) => {
      if(res.meta.error_code == AppStatusCode.StatusCode200) {
        this.lstServicePricing = res.data;
        this.GetlistFunctions(id);
        this.NamePricing = this.lstServicePricing.Name
