...
@Component({
  selector: 'app-resident-card-information',
  templateUrl: './resident-card-information.component.html',
  styleUrls: ['./resident-card-information.component.scss']
})
export class ResidentCardInformationComponent  {
  fResidentCard: any;
  ...
  setFormGroup(event: any) {
    ...
    if(event?.value == null) {
      ...
      this.Resident = this.lstResident.filter((i: any) => i.ResidentId == event.value)[0];
    }
  ...
  onSubmit() {
    if(this.fResidentCard.invalid){
      ...
      itemCard.CardOwner = this.lstResident.filter((item: any) => item.ResidentId == itemCard.ResidentId)[0].FullName;
      itemCard.CardResidentId = this.ResidentCard.CardResidentId;
      itemCard.CardNumber = this.ResidentCard.CardNumber;
      itemCard.change = true;
