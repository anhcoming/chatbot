...
export class ResidentMoveInComponent {
  fResidentMoveIn: any;
  public text : string = '';
  nameFile : string = '';
  ...
  ngOnInit() {
    ...
    if(this.lstResidentMoveIn){
      this.gender.map((item: any) => {
        if(item.Id == this.lstResidentMoveIn.Gender) {
          this.lstResidentMoveIn.Gender = item.Code;
        }
      })
      this.setFormGroup()
    }
  ...
  setFormGroup() {
    ...
    if(this.isLoading == false){
      Object.keys(this.fResidentMoveIn.controls).forEach(key => {
        const control = this.fResidentMoveIn.get(key);
        control.disable();
      });
    }
    this.gender.map((item: any) => {
      if(item.Code == this.fResidentMoveIn.get('Gender').value){
        item.checked = true;
      }else{
        item.checked = false;
      }
  ...
  onSubmit() {
    if(this.fResidentMoveIn.invalid){
      this.markAllAsDirty();
    }
    else{
      const ResidentMoveIns  = this.fResidentMoveIn.value;
      if(this.Idr){
        ResidentMoveIns.Id = this.Idr;
      }else{
        const objectId = new ObjectId();
        ResidentMoveIns.Id = objectId.toHexString();
      }
